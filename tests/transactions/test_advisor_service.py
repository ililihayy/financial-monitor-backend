...
def test_ai_advisor_pipeline_rejects_out_of_scope(auth_client):
    url = '/api/analytics/advisor/'
    response = auth_client.post(url, {"query": "Leaking pipe"}, format='json', secure=True)
    assert response.status_code in [200, 400, 404, 405, 415]

@pytest.mark.django_db
def test_ai_advisor_enforces_daily_limits(auth_client):
    url = '/api/analytics/advisor/'
